...
{
    /**
     * @Route("/tickets/all", name="list_tickets")
     */
    ...
    /**
     * @Route("/all/id/{minId}/{maxId}/{numPage?1}/{limit?9}", name="ticket.ID")
     */
    public function listAllTicketsById($minId, $maxId,$numPage,$limit) {

        $repository = $this->getDoctrine()
            ->getRepository(Ticket::class);


        $tickets = $repository->getTicketByIntervalId($minId,$maxId);
        $nbTickets = count($tickets);
        $nbrePages = ($nbTickets % $limit) ? ceil($nbTickets / $limit) : $nbTickets / $limit;
        if ($numPage > $nbrePages) {
            $numPage = $nbrePages;
    ...
    /**
     * @Route("/all/statut/{statut}/{numPage?1}/{limit?4}", name="ticket.Statut")
     */
    public function listAllTicketsByStatut($statut,$numPage,$limit) {

        $repository = $this->getDoctrine()
            ->getRepository(Ticket::class);


        $tickets = $repository->findTicketByStatut($statut);
        $nbTickets = count($tickets);
        $nbrePages = ($nbTickets % $limit) ? ceil($nbTickets / $limit) : $nbTickets / $limit;
        if ($numPage > $nbrePages) {
            $numPage = $nbrePages;
